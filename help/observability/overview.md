---
title: 観察可能性について
description: スケーラブルなAdobe Commerceの拡張性のために、一元化された観測可能性が重要な理由
feature: Observability, Extensibility, App Builder
topic: Architecture, Performance
role: Developer
level: Intermediate
doc-type: Technical Video
duration: 205
last-substantial-update: 2025-12-18T00:00:00Z
jira: KT-19965
exl-id: 619e7ac0-4415-4bea-829b-2211a3bf7d21
source-git-commit: 28257af422ceea62585d4f19ad7c81576c4a3653
workflow-type: tm+mt
source-wordcount: '120'
ht-degree: 0%

---

# Observability overview

最新のAdobe Commerce アーキテクチャにおいて、一元化されたオブザーバビリティが不可欠な理由をご確認ください。 拡張性が、独立したアプリ、webhook、UI拡張機能を備えた構成可能なモデルに進化するにつれ、マーチャントは複雑な実行フローを追跡するという課題に直面します。 統合された可観測性レイヤーがサービス間のシグナルを相関させ、先見的な最適化を可能にし、迅速で信頼性の高いチェックアウト体験を保証する方法を学びます。

## この動画は誰のためのものでしょうか？

* Adobe Commerce Developer
* ソリューションアーキテクト
* テクニカルプロダクトマネージャー

## ビデオコンテンツ

* コンポーザブルモデルで複雑さが増す：独立したアプリやwebhookには単一のコンテキストが欠如しています。
* 一元化された可観測性が重要：すべてのコンポーネントにわたってログと指標を関連付けます。
* チェックアウトパフォーマンスを高める：ショッピングをスムーズに保つために、ボトルネックをすばやく特定します。

>[!VIDEO](https://video.tv.adobe.com/v/3478286?learn=on)

{{$include /help/_includes/app-builder-observability-related-links.md}}

## 関連ドキュメント

* [ オブザーバビリティの概要](https://developer.adobe.com/commerce/extensibility/observability/){target="_blank"}
