---
title: Git グローバル参照アーキテクチャの分割
description: Split Git Global Reference Architectureを使用してAdobe Commerceを設定し、効率的なコード管理と合理的なデプロイメントを実現する方法について説明します。
jira: KT-16725
doc-type: Tutorial
duration: 330
...
* モジュール、テーマ、言語パック、composer-plugin、composer-metapackage、magento2-component、パッチなど、このモデルを通じて任意のComposer パッケージをインストールできます
...
* ほとんどの場合、バルクパッケージパターンは、同様に単純ですが、より従来のものであるため、より優れた代替手段です
...
これらの2つのリポジトリには、サードパーティパッケージとGRA パッケージが格納されます。 一部のコードは、Adobe Commerceの各インスタンスにのみ適用されます。 これらのローカルパッケージをgra-split-brand-x リポジトリに保存する場所を作成します。
...
Adobe Commerceは、Composer アプリケーションです。 インストールする推奨される方法は、常にComposer リポジトリを使用することです。 モジュールベンダーがComposer リポジトリを介したインストールを提供しない場合にのみ、サードパーティモジュールをサードパーティリポジトリに保存できます。 カスタムコードの優先される場所は、GRA リポジトリです。 ある特定のインスタンスがモジュールを使用すると、そのインスタンスはローカルコードになります。
...
GRA パターンの仕組みを確認するには、概念実証としてサンプルモジュールをインストールします。

次に進む前に`composer install`と`bin/magento install`を実行してください。

GitHubには3つのテストモジュールがあります。
...
この最後のコマンドは、モジュールがインストールされ、動作していることを示す次の出力になります。
...
`packages/3rdparty`、`packages/gra`、`packages/local` ディレクトリから直接変更をコミットできるようになりました。 Gitは、ディレクトリがシンボリックリンクするGit リポジトリに変更をコミットします。 Git commit コマンドは、`packages/3rdparty`、`packages/gra`または`packages/local` ディレクトリ内で発行することが重要です。 プロジェクトのルートでGit commitを実行しないでください。
...
上記の出力が表示された場合は、ブランドリポジトリに安全にコミットできます。 適切なリモートにコミットしていることを確認するには、`git remote -v`を実行します。
...
コードをAdobe Commerce インスタンスに配信するには、GRAとサードパーティのリポジトリをgra-split-brand-x リポジトリにマージします。 `composer require`、`bin/magento module:enable`を実行して、結果を確定します。
