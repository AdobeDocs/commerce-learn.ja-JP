...
2. すべてのデータベーステーブルの行形式が、COMPACT[&#128279;](https://experienceleague.adobe.com/en/docs/commerce-operations/implementation-playbook/best-practices/maintenance/mariadb-upgrade#convert-database-table-storage-format){target="_blank"}ではなくDYNAMICに設定されていることを確認します（オンプレミスからクラウドへの移行の場合は特に重要です）。
