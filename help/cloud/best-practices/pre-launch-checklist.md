...
2. すべてのデータベーステーブルの行形式が、COMPACT](https://experienceleague.adobe.com/en/docs/commerce-operations/implementation-playbook/best-practices/maintenance/mariadb-upgrade#convert-database-table-storage-format){target="_blank"}ではなく[DYNAMICに設定されていることを確認します（オンプレミスからクラウドへの移行の場合は特に重要です）。
